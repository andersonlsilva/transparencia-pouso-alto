...
st.set_page_config(page_title="Portal Transparência Pouso Alto", layout="wide", page_icon="🏦")
...
st.sidebar.info("Município: Pouso Alto - MG\n\nEntidade: Prefeitura Municipal\n\nData: 21/03/2026")
...
dispensas_raw = fetch_data(
    ...
    {'data_inicial': '', 'dta_final': '', 'ano': '2026-03-21', 'id_entidade': '2', 'situacao_processo_compra': '0'}
)

# --- SEÇÃO 1: RESUMO DE DESPESAS ---
if despesas_raw:
    # 1. Criação do DataFrame e Verificação de Estrutura
    if isinstance(despesas_raw, dict) and 'data' in despesas_raw:
        df_desp = pd.DataFrame(despesas_raw['data'])
    else:
        df_desp = pd.DataFrame(despesas_raw)

    # 2. Identificação Automática de Colunas (Onde estava o erro)
    colunas = df_desp.columns.tolist()
    
    # Busca por nomes comuns de Valor e Credor
    c_valor = next((c for c in ['vl_empenhado', 'vlr_empenhado', 'valor', 'vl_liquido'] if c in colunas), None)
    c_credor = next((c for c in ['nm_credor', 'nome_credor', 'credor', 'nm_pessoa'] if c in colunas), None)

    # Fallback: Se não achou os nomes, tenta por tipo de dado
    if not c_valor:
        for c in colunas:
            if "vl" in c.lower() or "valor" in c.lower():
                c_valor = c
                break
    if not c_credor:
        for c in colunas:
            if "nm" in c.lower() or "nome" in c.lower():
                c_credor = c
                break
    ...
    if c_valor and c_credor:
        ...
        col3.metric("Maior Lançamento", f"R$ {maior_gasto:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
        ...
        with col_detalhe:
            ...
            credor_sel = st.selectbox("Selecione para detalhar:", sorted(df_desp[c_credor].unique()))
            ...
            c_data = next((c for c in ['dt_documento', 'data', 'dt_emissao'] if c in colunas), colunas[0])
            c_hist = next((c for c in ['ds_historico', 'historico', 'observacao'] if c in colunas), None)
        ...
        st.error(f"Erro: Não identificamos as colunas de dados. Colunas disponíveis: {colunas}")
...
if dispensas_raw:
    ...
    c_modalidade = next((c for c in ['ds_modalidade', 'modalidade', 'tipo'] if c in cols_disp), None)
    c_objeto = next((c for c in ['ds_objeto', 'objeto', 'descricao'] if c in cols_disp), None)
    c_data_disp = next((c for c in ['dt_processo', 'data', 'dt_abertura'] if c in cols_disp), None)
    ...
    if c_modalidade:
        ...
        with c2:
            ...
            colunas_mostrar = [c for c in [c_data_disp, c_objeto, c_modalidade] if c is not None]
            df_disp_clean = df_disp[colunas_mostrar].copy()
            
            # Renomear para o cidadão entender
            nomes_amigaveis = {c_data_disp: 'Data', c_objeto: 'Objeto/Finalidade', c_modalidade: 'Modalidade'}
            df_disp_clean.rename(columns=nomes_amigaveis, inplace=True)
            
            st.dataframe(df_disp_clean, use_container_width=True, hide_index=True)
    else:
        st.warning(f"Dados de dispensas recebidos, mas o formato é inesperado. Colunas: {cols_disp}")
else:
